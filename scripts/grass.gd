...
func _ready() -> void:
	...
	if mat:
		mat.set_shader_parameter("offsdet", randf_range(0.0, 10.0))
...
func _on_body_entered(body: Node2D) -> void:
	...
	var skew = clamp(remap(200 * sign(-direction.x), -200, 200, min_skew, max_skew), min_skew, max_skew)
	...
	if tween:
		tween.tween_property(mat, "shader_parameter/skew", skew, 0.3).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
